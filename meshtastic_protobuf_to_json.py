...
Subscribes to Protobuf MQTT topics, converts packets to JSON, and prints them.
"""
...
class MeshtasticConverter:
    def __init__(self, broker, port, username, password, root_topic, UUID, psk=None):
        #        args.broker, args.port, args.username, args.password, args.root_topic, args.UUID, args.psk
        self.broker = broker
        self.port = port
        self.UUID = UUID
    ...
    def on_connect(self, client, userdata, flags, rc, properties=None):
        if rc == 0:
            protobuf_topic = f"{self.root_topic}/2/e/+/!{self.UUID}"
            #json_topic = f"{self.root_topic}/2/json/#"
...
#           client.subscribe(json_topic)
    ...
    def on_message(self, client, userdata, msg):
        try:
            ...
            if '/e/' in msg.topic:
                print(f"Incoming message! {msg.topic}")
    ...
    def start(self):
        logger.info(f"Starting converter for UUID {self.UUID}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--UUID', default='#', help='User UUID')
